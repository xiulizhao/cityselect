(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.cityselect) {
      window.wwload.cityselect = "wait";
      requirejs.config({
        paths: {
          "cityselect": "libs/cityselect/js/jquery.cityselect" // 示例: libs/qrcodejs/qrcode
        }
      });
      require(["cityselect"], function () {
        window.wwload.cityselect = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.cityselect === "wait") {
      setTimeout(function () {
   ...
   */
  // parsePlace == parseaddress
  // placeStr == addressStr
  // placeObj == addressObj
  // getPlaceStr == getaddressStr
  ...
  function parseaddress(addressStr) {
    ...
    addressObj.provcity = addressStr[0];
    addressObj.cityarea = addressStr[1];

    return addressObj;
  }


  function getaddressStr($ele) {
    var addressStr = [];
    $ele.find("select").each(function (index, targetDom) {
      addressStr.push($(targetDom).val());
    });
  ...
  function processElement($ele) {
    ...
    // 对 $ele 元素做对应处理
    // var config = {};
    // config.provcity = $ele.attr("data-provcity");
    // config.cityarea = $ele.attr("data-cityarea");
    //console.log(config.provcity);
    var addressObj = parseaddress($ele.attr("data--cityname"));
    //console.log(addressObj);

    var place = addressObj;
    
    place.provcity = $ele.attr("data-provcity");
    place.cityarea = $ele.attr("data-cityarea");
    
    for (var name in addressObj) {
      $.wwclass.helper.updateProp($ele, "data-x-" + name, addressObj[name]);
    }
    $ele.citySelect({ nodata: "none", required: false, prov: place.provcity, city: place.cityarea });
    $ele.on("change", "select", function (e) {
      var $ele = $(e.delegateTarget);
      var $select = $(e.currentTarget);
      // 改变控制属性, 内部改变控制属性 添加data标识
      $ele.data("interne1", true);
      var addressStr = getaddressStr($ele);
      $.wwclass.helper.updateProp($ele, "data--cityname", getaddressStr($ele));
      // 改变输出属性
      var addressObj = parseaddress(addressStr);
      for (var name in addressObj) {
        $.wwclass.helper.updateProp($ele, "data-x-" + name, addressObj[name]);
      }
      // 发出数据更改的输出事件
  ...
  var evtInHandler = function ($ele, attribute, value) {
    switch (attribute) {
      case "data--cityname":
        if ($ele.data("interne1")) {
          $ele.data("interne1", false);
          return;
        }
        $ele.citySelect("destroy");
        var addressObj = parseaddress($ele.attr("data--cityname"));
        var place = addressObj;
